...
# 2 面向对象与设计模式
## 2.1 面向对象编程
Python世界的对象：对象是一个数据以及相关行为的集合
类是对象描述的一种抽象，对象是实体

Python的经典类与新式类：Python2.7后新式类开始成熟，Python3中全是新式类

类的两大成员：属性和方法

### 2.1.1 属性
创建类的方式不唯一，并不是只有使用class关键字这一种方式。
类属性：类属性字段在内存中只保存一份
对象属性：对象属性在每个对象都保存一份
...
简单工厂模式：工厂相当于公司，要实现两种角色：具体的产品角色和抽象的产品角色
工厂：传入不同的参数，返回不同类的实例
动态属性工厂模式
...
# 2 优化运行效率
现实生活中需要考虑程序的运行环境。比如在服务器上，不是单核心，如何利用多核。
## 2.1 多进程
在Linux中，进程只有一个虚拟内存的示例，只能占用一个虚拟处理器。多个线程共享地址空间。
进程的创建由内核完成：
- fork()
- exec()

将父进程运行为守护进程(daemon)，则自己成会随父进程一起消失
管道和队列用于进程间通信

python中的多进程和多线程间的通信优先使用队列

## 2.2 多线程
### 2.2.1 同步 vs 异步

同步和异步描述的是被调用方，一般用于消息的通信机制
同步：得到结果之前，调用不会返回
异步：请求发出后，调用立即返回，但没有返回结果，通过回调函数获得实际结果
...
Python2.7以下版本，在没有IO等待情况下，是每100个始终滴答进行切换
Python2.7及Python3 版本，5ms切换一次
如果是多线程做CPU密集型任务，解决方法：
- 换解释器
- 使用C/C++扩展
...
条件锁：threading.Condition()满足某些条件释放阻塞着的锁
信号量：较少用，一般使用线程池
